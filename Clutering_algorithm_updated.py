    ...
    with pd.ExcelWriter(output_file_path, engine='openpyxl') as writer:
        for sheet_name, df in excel_data.items():
            if sheet_name == "Statistics":
                ...
                check_for_file("subplans_path.txt")
                with open("subplans_path.txt" , 'w') as file :
                    file.write(f"{os.path.join(os.getcwd(),time_stamp)}")
                    file.close()
                ...
                for suite in unique_suites:
                    ...
                    for clusters in prioritized_clusters:
                        ...
                        if my_xml_str != 0 and cluster_wise_count > 0:
                            ...
                            with open(os.path.join(os.getcwd(), time_stamp, f"{suite}", f"P-{prioritized_clusters.index(clusters)}_{cluster_wise_count}.xml"), 'w') as f:
                                f.write(my_xml_str)
